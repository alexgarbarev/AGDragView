...
#import "ZBDragView.h"
#import "ZBEpisodesScrollView.h"

typedef enum {
    ZBDragViewStateMinimized = 0, ZBDragViewStateFullscreen
} ZBDragViewState;

@interface ZBDragView () <UIScrollViewDelegate>

@property (nonatomic) ZBDragViewState state;

@end

@implementation ZBDragView {
    ZBEpisodesScrollView *_scrollView;
...
- (void) commonInit
{
    _scrollView = [[ZBEpisodesScrollView alloc] initWithFrame:self.bounds];
...
- (void)setDelegate:(id<ZBDragViewDelegate>)delegate
{
    _delegate = delegate;
